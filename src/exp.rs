...
struct Graph<T: Copy, CS: Consumer<T> + Send> {
    consumable_indices: Vec<AtomicI64>,
...
impl <T: Copy + Send + Sync, CS: Consumer<T> + Send + Sync> Graph<T, CS> {
    fn new() -> Graph<T, CS> {
        ...
        Graph{consumable_indices: vec![], consumers: vec![], transformers: vec![], consumers_deps: vec![], transformers_deps: vec![],
            inner: Arc::new(inner)}
    }
    fn register_producer(&mut self) -> RegistrationHandle<T> {
        self.consumable_indices.push(AtomicI64::new(0));
        RegistrationHandle{upstream_index: 0, d: PhantomData{}}
    }
    ...
    fn produce(& self, data: Vec<T>) -> () {
        ...
        */
        let cur_idx = self.consumable_indices.get(0).unwrap().load(Ordering::Acquire);
        let new_idx = cur_idx + (data.len() as i64);
        loop {
            let has_conflict = self.consumable_indices.iter().any(|idx| idx.load(Ordering::Relaxed) >= new_idx);
            if !has_conflict {
                let d_size = data.len();
                self.write(cur_idx as usize % RING_SIZE, data);
                self.consumable_indices.get(0).unwrap().fetch_add(d_size as i64, Ordering::Release);
                break;
            } else {
        ...
        thread::scope(|s|{
            ...
            */
            for i in 0..self.consumers.len() {
                ...
                s.spawn(|| {
                    let mut consumer_idx: i64 = 0;
                    loop {
                        let upstream_load = upstream.load(Ordering::Acquire);
                        if consumer_idx < upstream_load {
                            let starting: usize = consumer_idx as usize % RING_SIZE;
                            let end: usize = upstream_load as usize % RING_SIZE;
...
struct RegistrationHandle<T>{
    upstream_index: usize,
    d: PhantomData<T>
}

impl <T: Copy> RegistrationHandle<T> {
    fn register_consumer<CS: Consumer<T> + Send>(&self, graph: &mut Graph<T, CS>, consumer: CS) -> () {
        graph.consumers.push(Box::new(consumer));
        graph.consumers_deps.push(self.upstream_index);
    }
    fn register_transformer<CS: Consumer<T> + Send>(&self, graph: &mut Graph<T, CS>, transformer: impl Transformer<T> + 'static + Sync + Send) -> Self {
        graph.transformers.push(Box::new(transformer));
        graph.transformers_deps.push(self.upstream_index);
        RegistrationHandle{upstream_index: graph.transformers.len() - 1, d: PhantomData{}}
    }
}
